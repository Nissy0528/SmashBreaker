...
{
    private ParameterMenu menu;//メインメニュー
    //private PositionParameter positionsSet;
    private Vector2 scrollPos = Vector2.zero;//スクロール座標
    private Rect windowRect;//ウィンドウ
    private Object bossFile;
    private int bossCout;//ボスの数
    ...
    /// シーン上に表示
    /// </summary>
    public BossParameter()
    {
        //this.positions = new List<Vector2>();
        //ReadExcel(positions);
        SceneView.onSceneGUIDelegate += BossOnSceneGUI;
        //bossFile = Resources.Load("Prefab/Boss");
        //string path = AssetDatabase.GetAssetPath(bossFile);
        //string[] names = Directory.GetFiles(path, "*.prefab");
        //bossCout = names.Length;
    }
